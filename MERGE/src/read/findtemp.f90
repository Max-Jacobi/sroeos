...
SUBROUTINE findtemp(lr,lt0,y,epsin,keyerrt,rfeps)
  ...
  USE Table_Sizes_Mod, ONLY : nrho,ntemp,nyp,nvars,logrho,logtemp,yp,&
                              t_max_hack,alltables
  IMPLICIT NONE
...
!  write(*,"(i4,1P12E19.10)")
!  0,lr,lt0,y,lt,eps,eps0,abs(eps-eps0)/eps0,d2
...
!     if(ldt.le.0.d0) ltmax=lt
     ...
     eps1=eps
!     write(*,"(i4,1P12E19.10)")
!     i,lr,lt0,y,lt,eps,eps0,abs(eps-eps0)/eps0,d2,ldt
     call findthis(lr,lt,y,eps,alltables(:,:,:,2),d1,d2,d3)
...
!       write(*,*)
!       "rl,logrho,logtemp0,ye,lt,eps,eps0,abs(eps-eps0)/eps0"
!     if(i.gt.5) then
!       write(*,"(i4,1P10E22.14)")
!       i,lr,lt0,y,lt,eps,eps0,abs(eps-eps0)/eps0
!     endif
  enddo
...
! on rho, Y_e, entropy
  ...
  tol=rfeps ! need to find energy to less than 1 in 10^-10
  itmax=50 ! use at most 20 iterations, then bomb
