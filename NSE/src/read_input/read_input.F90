...
!   read input file
  ...
  SUBROUTINE READ_TEST_INPUT
    ...
    NAMELIST /INPUT_TEST_LIST/ DENSITY,TEMPERATURE,PROTON_FRACTION
    ...
    IF (DENSITY < 1.D-17) THEN
      WRITE (*,*) 'Code is unreliable for densities n < 1.d-17 fm^-3'
      CALL CONTINUE_EXCUTION()
    ENDIF

    IF (DENSITY > 10.D0) THEN
      WRITE (*,*) 'Code is unreliable for densities n > 0.1d0 fm^-3'
      CALL CONTINUE_EXCUTION()
    ENDIF
